...
async function createUser(user) {
  ...
  try {
    connection = await sql.connect(dbConfig);
    const query =
      "INSERT INTO Users (name, email) VALUES (@name, @email); SELECT SCOPE_IDENTITY() AS id;";
    const request = connection.request();
...
async function updateUser(id, updatedUser) {
  ...
  try {
    ...
    const request = connection.request();
    request.input("id", id);
    request.input("name", updateUser.name);
    ...
    return await getUserById(id); // Return updated book
  } catch (error) {
    console.error("Database error:", error);
    throw error;
  } finally {
